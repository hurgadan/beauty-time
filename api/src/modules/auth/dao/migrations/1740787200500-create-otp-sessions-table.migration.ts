...
export class CreateOtpSessionsTable1740787200500 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.createTable(
      new Table({
        ...
        columns: [
          ...
          { name: 'purpose', type: 'varchar', isNullable: false, default: "'booking_confirm'" },
    ...
    await queryRunner.createForeignKey(
      'otp_sessions',
      new TableForeignKey({ name: 'fk_otp_sessions_tenant_id', columnNames: ['tenant_id'], referencedTableName: 'tenants', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
    );
    await queryRunner.createForeignKey(
      'otp_sessions',
      new TableForeignKey({ name: 'fk_otp_sessions_client_id', columnNames: ['client_id'], referencedTableName: 'clients', referencedColumnNames: ['id'], onDelete: 'SET NULL' }),
    );
  }
