...
describe("Appointments/Staff/Services (e2e)", () => {
  it("POST /api/crm/services creates service and GET /api/crm/services returns tenant-only data", async () => {
    const app = getE2eApp();
    ...
    expect(listResponse.body.items[0].name).toBe("Visible TenantA Service");
    expect(listResponse.body.items[0].tenantId).toBe(tenantA.id);
  });
  ...
  it("GET /api/crm/staff and /api/crm/appointments support filtering and limit", async () => {
    const app = getE2eApp();
    const dataSource = getE2eDataSource();
    ...
    const staffResponse = await request(app.getHttpServer())
      .get("/api/crm/staff?search=anna&limit=1")
      .set("Authorization", `Bearer ${token}`)
      .expect(200);

    expect(staffResponse.body.items).toHaveLength(1);
    expect(staffResponse.body.items[0].fullName).toBe("Anna Muller");

    const appointmentsResponse = await request(app.getHttpServer())
      .get(
        `/api/crm/appointments?staffId=${staffAnna.id}&status=${AppointmentStatus.BOOKED}&limit=1`,
      )
      .set("Authorization", `Bearer ${token}`)
      .expect(200);

    expect(appointmentsResponse.body.items).toHaveLength(1);
    expect(appointmentsResponse.body.items[0].staffId).toBe(staffAnna.id);
    expect(appointmentsResponse.body.items[0].status).toBe(
