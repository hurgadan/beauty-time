...
describe("BookingAppointmentsService", () => {
  ...
  it("listAppointments passes query filters and limit to repository", async () => {
    ...
    expect(bookingAppointmentsRepository.findAppointments).toHaveBeenCalledWith(
      "tenant-id",
      query,
    );
  });
  ...
  it("createAppointment throws BadRequestException when start >= end", async () => {
    ...
    await expect(
      service.createAppointment("tenant-id", payload),
  ...
  it("createAppointment throws ConflictException on overlapping slot", async () => {
    ...
    bookingAppointmentsRepository.hasAppointmentOverlap!.mockResolvedValue(
      true,
    );
  ...
  it("createAppointment saves and returns mapped appointment", async () => {
    ...
    bookingAppointmentsRepository.saveAppointment!.mockImplementation(
      async () => ({
        id: "appointment-id",
        tenantId: "tenant-id",
        staffId: "staff-id",
