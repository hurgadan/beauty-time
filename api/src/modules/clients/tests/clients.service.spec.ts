...
type ClientsRepositoryMock = Partial<Record<keyof ClientsRepository, jest.Mock>>;
...
describe('ClientsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ClientsService(clientsRepository as unknown as ClientsRepository);
  });
  ...
  it('getClient throws NotFoundException when client does not exist', async () => {
    ...
    await expect(service.getClient('tenant-id', 'client-id')).rejects.toBeInstanceOf(
      NotFoundException,
    );
  });
  ...
  it('getClient returns mapped client and returning flag based on visit count', async () => {
    ...
    expect(service.toClientDto(result.client, result.visitCount).isReturningClient).toBe(true);
  });
  ...
  it('listClientHistory returns mapped items', async () => {
    ...
    const result = await service.listClientHistory('tenant-id', 'client-id', 20);
