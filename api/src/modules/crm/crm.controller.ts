...
export class CrmController {
  ...
  public listAppointments(@Req() request: RequestWithUser): ListAppointmentsResponseDto {
  ...
  public listServices(@Req() request: RequestWithUser): ListServicesResponseDto {
    const tenantId = request.user?.tenantId;
    if (!tenantId) {
      throw new UnauthorizedException('Missing tenant in token');
    }
